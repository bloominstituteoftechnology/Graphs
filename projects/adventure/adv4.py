...
while stack.size() > 0:
    ...
    if '?' in string:
        direction = []
        # Mark it as visited...
        for d in traversalGraph[v]:
            if traversalGraph[v][d] == '?':
                direction.append(d)
            
            
        move = random.choice(direction)
        if move == 'n':
            traversalGraph[v]['n'] = roomGraph[v][1]['n']
            player.travel('n')
            ...
            traversalGraph[v]['s'] = roomGraph[v][1]['s']
            player.travel('s')
            traversalPath.append(move)
            current = player.currentRoom.id
            traversalGraph[current]['n'] = roomGraph[current][1]['n']
        elif move == 'e':
            traversalGraph[v]['e'] = roomGraph[v][1]['e']
            player.travel('e')
            traversalPath.append(move)
            current = player.currentRoom.id
            traversalGraph[current]['w'] = roomGraph[current][1]['w']
        elif move == 'w':
            traversalGraph[v]['w'] = roomGraph[v][1]['w']
            player.travel('w')
            traversalPath.append(move)
            current = player.currentRoom.id
        ...
        for i in roomGraph[v][1]:
            # print(roomGraph[v][1][i])
            stack.push(roomGraph[v][1][i])
