...
# Returns a tuple of the abs difference between two coordinate points
def coordinateDifference(fromCoor, toCoor):
...
def numberOfValidExits(room, ignoring):
    count = 0
    for next in room.getExits():
        player.travel(next)
        if player.currentRoom is not ignoring:
            count += 1
        player.travel(oppositeDirection(next))
    return count

def unvisitedExits(room, visited):
    unvisited = []
    for r in room.getExits():
        player.travel(r)
...
def getDirectionToRoom(fromRoom, toRoom):

    for dir in fromRoom.getExits():
        player.travel(dir)
        if player.currentRoom == toRoom:
            player.travel(oppositeDirection(dir))
            return dir
        else:
            player.travel(oppositeDirection(dir))
