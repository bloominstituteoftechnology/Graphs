...
# keep stack of rooms with unexplored [n,s,e,w] aka directions
# when at dead end return to TOS room
# for each entry appended to tranversalPath append the roomId moved to 
# bfs of adjacency dictionary
...
def inverse(c):
    if c == 'e':
        ...
        raise IndexError('direction not in "nsew"')

def randomDirection(roomId, lastMove):
    exits = world.rooms[roomId].getExits()
    print('randomDirection roomId', roomId, 'lastMove', lastMove, 'exits', exits)    
    if lastMove is not None:
        del exits[exits.index(inverse(lastMove))]
    if len(exits) == 0:
        return None
...
def bfs(roomId):
    visited = set()
    q = Queue()
    q.enqueue([(roomId, None)])
    while q.size() > 0:
        path = q.dequeue()
        v = path[-1]
        if has_hook(v[0]) is not None:
            for i in range(len(visited) - 1, 0, -1):
                traversalPath.append(inverse(path[i][1]))
            return v[0]
        if v not in visited:
            visited.add(v)
            for c in 'nsew':
                q.enqueue([*path, (am[v][c],c)])
    return None   
...
try:
    while True:
        d = randomDirection(roomId, lastMove)
        if d == None:
            print('d is None')
            roomId = bfs(roomId)
            if roomId == None:
            ...
            nroomId = world.rooms[roomId].getRoomInDirection(d).id
            print(f"d: {d} roomId: {roomId} nroomId: {nroomId}")
            am[roomId][d] = nroomId
            if nroomId not in am:
                am[nroomId] = empty
            am[nroomId][inverse(d)] = roomId
            roomId = nroomId
