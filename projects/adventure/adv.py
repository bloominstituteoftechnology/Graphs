...
empty = {'n': None, 's': None, 'w': None, 'e': None}
am = {
  0: copy.deepcopy(empty)
}
for c in world.rooms[0].getExits():
    am[0][c] = '?'

def randomDirection(roomId):
    exits = []
    for c in am[roomId]:
        if am[roomId][c] == '?':
            exits.append(c)
...
def bfs(roomId):
    ...
    # print('bfs roomId', roomId)
    while q.size() > 0:
        path = q.dequeue()
        v = path[-1]
        if has_hook(v[0]) is not None:
            # print('has hook for', v)
            # print('bfs hook found path', path)
            for i in range(1, len(path)):
                traversalPath.append(path[i][1])
                roomPath.append(path[i][0])
            return v[0]
        if v not in visited:
            visited.add(v)
            for c in 'nsew':
                if am[v[0]][c] is not None:
                    q.enqueue([*path, (am[v[0]][c],c)])
    return None   
    ...
    while True:
        ...
        if d == None:
            ...
            if nroomId not in am:
                ...
                for c in nroom.getExits():
                    am[nroomId][c] = '?'
                # print('added', nroomId,'to am')
            am[nroomId][inverse(d)] = roomId
            # print(nroomId, 'final nroomId am', am[nroomId])
            roomId = nroomId
            traversalPath.append(d)
            roomPath.append(roomId)
            lastMove = d
        loop -= 1
        if loop == 0:
            print('loop exit')
