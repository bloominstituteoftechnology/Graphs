...
#NOTES
# I dont think you need to create a class or a bunch of functions. Lets use what we have.
# getAllSocialPaths in social.py
# DFT to BFS
# What do I want? I want to create a dictionary. The key will be the ROOM NUMBER. The value will be another dictionary that contains the keys: "n", "s", "e", "w". The values will be the room number (if it exists) that is in that direction.
'''
{
0: {'n': '?', 's': 5, 'w': '?', 'e': '?'},
5: {'n': 0, 's': '?', 'e': '?'}
...
'''
# I know I'm finished whem my traversal_graph contains 500 rooms.
...
traversalPath = ['n', 's']  # tracker for dft # keeps track of your moves
visited = {}    # dictionary of rooms you have visited
tracker = [] # add rooms to tracker
tracker.append(player.currentRoom.id)
...
print(f"TEST", visited)
# keep running until dict is 
# while len(visited) < 9:
print(f"START: ", player.currentRoom.id)
while len(visited) < 3:
    for direction in visited[player.currentRoom.id]:
        # if direction value not in visited
        print(f"before FOR: ", player.currentRoom.id)
        player.travel(direction)  
        print(f"after FOR: ", player.currentRoom.id)
        # add room num to tracker
        tracker.append(player.currentRoom.id)
        # add direction to traversePath
        traversalPath.append(direction)
        # add room to dictionary
        visited.update({player.currentRoom.id: {i: player.currentRoom.getRoomInDirection(i).id for i in player.currentRoom.getExits()}})
        

        print(f"TRACKER LIST", tracker)
        print(f"TRAVERSAL PATH", traversalPath)
        # if player.currentRoom.id not in visited and player.currentRoom.id not in tracker:
        #     print(visited)
        #     player.travel(direction)
        #     print(f"IF: ", player.currentRoom.id)
        #     visited.update(
