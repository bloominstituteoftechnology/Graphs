...
while len(graph) < len(roomGraph):
    ...
    for direction in graph[room]:
        if graph[room][direction] == '?':
            newdir = direction
            if newdir:
                traversalPath.append(newdir)
                player.travel(newdir)
                newroom = player.currentRoom.id
                if newroom not in graph:
                    add_room(newroom)
            # Update graph
            graph[room][newdir] = newroom
            graph[newroom][opp_dir(newdir)] = room
            # Update room for bfs
            room = newroom
            break
    path = bfs(room)
    if path:
        for neighbor in path:
            for direction in graph[room]:
                if graph[room][direction] == neighbor:
