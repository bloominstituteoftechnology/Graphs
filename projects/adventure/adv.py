...
def path_seeking(player):
    ...
    def helper(player, room):
        ...
        for key in graph_map[room.id]:
            if (graph_map[room.id])[key] != '?' and len(exits) > 0:
                exits.remove(key)
        print(f"unexplore exits {exits}")
        if len(exits) > 0:
            if exits[0] == 'n':
                opposite = 's'
            if exits[0] == 's':
        ...
        while len(exits) > 0:
            ...
            path.append(exits[0])
            print(f"moving direction: {exits[0]}")
            print(f"move back direction: {opposite}")
            (graph_map[room.id])[exits[0]] = player.currentRoom.name
            exits.pop(0)
            (graph_map[player.currentRoom.id])[opposite] = room.name
            print(graph_map)
            print(f"steps: {steps}")
            helper(player, player.currentRoom)
        if len(exits)==0:
            print('else xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx')
            headback=''
            checker=False
            while not checker:
                print('*****************************************')
