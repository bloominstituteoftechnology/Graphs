...
def check_east(current=player.currentRoom.id):
    global counter
    checkpoint = current
    print('east is running')
    if 'e' in traversalGraph[current]:    
        if traversalGraph[current]['e'] == '?' : #if not visited
            counter = 0
            traversalGraph[current]['e'] = roomGraph[current][1]['e'] #set that ? to the number about to be visited
            player.travel('e') #visit that number
            traversalPath.append('e') #add that number to path
            reversePath.append('w')
            fastPath.append('w')
            current = player.currentRoom.id #change the current room variable
            traversalGraph[current]['w'] = roomGraph[current][1]['w'] #change the ? variable names in the new room 
...
def check_south(current=player.currentRoom.id):
    ...
    if 's' in traversalGraph[current]:    
        if traversalGraph[current]['s'] == '?' : #if not visited
            ...
            traversalPath.append('s') #add that number to path
            reversePath.append('n')
            fastPath.append('n')
            current = player.currentRoom.id #change the current room variable
            traversalGraph[current]['n'] = roomGraph[current][1]['n'] #change the ? variable names in the new room 
            # print(traversalGraph)
            visited_rooms.add(current) #add rooms to visited
            print(f'player currently in {current}')
            print('calling west')
...
def check_west(current=player.currentRoom.id):
    ...
    if 'w' in traversalGraph[current]:    
        if traversalGraph[current]['w'] == '?' : #if not visited
            counter = 0
            traversalGraph[current]['w'] = roomGraph[current][1]['w'] #set that ? to the number about to be visited
            player.travel('w') #visit that number
            traversalPath.append('w') #add that number to path
            reversePath.append('e')
            fastPath.append('e')
            current = player.currentRoom.id #change the current room variable
            traversalGraph[current]['e'] = roomGraph[current][1]['e'] #change the ? variable names in the new room 
            # print(traversalGraph)
            visited_rooms.add(current) #add rooms to visited
            print(f'player currently in {current}')
            print('calling north')
...
def counter_check_north(current):
    ...
    #             current = player.currentRoom.id
    #             print(f'player traveled west - you are at {current}')
    #             print(f'traversal graph {traversalGraph[current]} ')
    #             string = json.dumps(traversalGraph[current])
    #             if 'n' in string and '?' in string:
    #                 return check_north(current)
    #             elif 'n' in string:
    #                 while '?' not in string:
    #                     player.travel('n')
    ...
    #             current = player.currentRoom.id
    #             print(f'player traveled east - you are at {current}')
    #             print(f'traversal graph {traversalGraph[current]} ')
    #             string = json.dumps(traversalGraph[current])
    #             if 's' in string and '?' in string:
    #                 return check_south(current)
    #             elif 's' in string:
    #                 while '?' not in string:
    #                     player.travel('s')
    ...
    #                         return check_west(current)
    #             if 'e' not in string:
    #                 full_string = json.dumps(traversalGraph)
    #                 if '?' in full_string:
    #                     return go_back(current)
    #                 print('youre done')
    #                 return visited_rooms
    ...
    # else:
    #     return check_west(current)
    # ['e', 'n', 'n', 's', 'w', 'n', 'w', 'w', 's', 'n', 'e', 'e', 's', 'e', 'n', 's', 'e', 
    # 's', 's', 'w', 's', 's', 's', 's', 's', 'w', 'n', 'w', 'e', 's', 'e', 'n', 'n', 'n', 'n', 'n', 
    # 'e', 'n', 'n', 'w', 'n', 's', 'w', 'n', 'w', 'w', 's', 'n', 'e', 'e', 's', 'e', 'n', 's']
# {0, 1, 2, 3, 4, 5, 6, 134, 8, 144, 16, 21, 157, 36, 41, 62, 65, 76, 218, 110, 118}
...
def counter_check_west(current): 
    ...
    #             traversalPath.append('n') 
    #             current = player.currentRoom.id
    #             print(f'player traveled north - you are at {current}')
    #             print(f'traversal graph {traversalGraph[current]} ')
    #             string = json.dumps(traversalGraph[current])
    #             if 'e' in string and '?' in string: 
    #                 return check_north(current)
...
def go_back(current):
    ...
    global fastPath
    counter = 0
    print('youve reached a dead end')
    copy = reversePath.copy()
    copy.reverse()
    copyPath = fastPath.copy()
    copyPath.reverse()
    full_string = json.dumps(traversalGraph)
    ...
    if '?' not in full_string:
        ...
        for x in copyPath:
            ...
            small_string = json.dumps(traversalGraph[current])
            if '?' in small_string:
                if x == 's':
                    print(current)
                    return check_west(current)
    ...
    # print(backwards_path)

    # if '?' in full_string:
    #     for room in backwards_path:
    #         player.travel(f'{room}')
    #         traversalPath.append(f'{room}')  
    #         current = player.currentRoom.id
    #         small_string = json.dumps(traversalGraph[current])
    #         if '?' in small_string:
    #             if traversalPath[-1] == 'n':
    #                 return check_west(current)
    #             elif traversalPath[-1] == 's':
    #                 return check_east(current)
    #             elif traversalPath[-1] == 'w':
    #                 return check_south(current)
    #             elif traversalPath[-1] == 'e':
    #                 return check_north(current)
...
# {0, 3, 4, 36, 69, 72, 41, 104, 105, 76, 21, 89}
# {0, 3, 4, 36, 69, 72, 41, 104, 105, 76, 21, 89}
# {0, 129, 3, 4, 260, 266, 274, 21, 36, 41, 190, 69, 72, 76, 89, 222, 225, 226, 104, 105, 379}
# ['s', 'e', 'n', 'e', 'e', 'n', 'w', 'w', 'w', 'e', 'e', 'e', 's', 'e', 'n', 'n', 'n', 'e', 's', 'n', 'e', 'e', 's', 'e', 'n', 's']
# West is running
# def counter_check_south(current):
#     string = json.dumps(traversalGraph[current])
#     print(string)
...
#                 traversalPath.append('w')
#                 current = player.currentRoom.id
#                 print(f'player traveled south - you are at {current}')
#                 print(f'traversal graph {traversalGraph[current]} ')
#                 string = json.dumps(traversalGraph[current])
#                 if 'w' in string and '?' in string:
#                     return check_east(current)
...
#     string = json.dumps(traversalGraph[current])
#     print(string)
#     if '?' not in string and 'ee' not in string:
#         if 'e' not in string:
#             while '?' not in string:
#                 player.travel('s')
#                 traversalPath.append('s')
