...
# Initialize live path tracker
curPath = {}
curPath.update({ player.currentRoom.id:{} })
exits1 = player.currentRoom.getExits()
for d in exits1:
    curPath[player.currentRoom.id].update( {d:'?'})
priorRoom = 0
check = set()
rooms = set(roomGraph.keys())
print(rooms)

# Function to search and fill traversal path
def curPathUpdate(curRoom):
    global priorRoom, check
    print(player.currentRoom.id)
    print(curPath)
    exits = list(curPath[curRoom].keys())
    nRoom = roomGraph[curRoom][1][exits[0]]
    print(nRoom)
    print(exits)
    if len(exits) == 1:
        print(check)
        if nRoom not in check:
            traversalPath.append(exits[0])
            player.travel(exits[0])
            curPath[curRoom].pop(exits[0])
            priorRoom = curRoom
            del curPath[priorRoom]
            curRoom = player.currentRoom.id
            if curRoom not in list(curPath.keys()):
                curPath.update({ curRoom:{} })
                exits = player.currentRoom.getExits()
                for d in exits:
                    curPath[curRoom].update( {d:'?'})
        else:
            traversalPath.append(exits[0])
            player.travel(exits[0])
            curPath[curRoom].pop(exits[0])
            ...
            print('Path Taken: ', traversalPath)
            print('~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~')
            return
    else: 
        print('Prior: ', priorRoom)
        print('Cur: ', curRoom)
        print(nRoom)
        if nRoom == priorRoom:
            traversalPath.append(exits[1])
            player.travel(exits[1])
            curPath[curRoom].pop(exits[1])
        else:
            traversalPath.append(exits[0])
            player.travel(exits[0])
            curPath[curRoom].pop(exits[0])
        # temp1 = {exits[0]: player.currentRoom.id}
        # curPath[priorRoom].update(temp1)
        # curPath[priorRoom].pop(exits[0])
        priorRoom = curRoom
        curRoom = player.currentRoom.id
        if curRoom not in list(curPath.keys()):
            curPath.update({ curRoom:{} })
        ...
        print(traversalPath)
        print(curPath[priorRoom], curPath[curRoom])
        print(exits)
        print(len(exits))
        print('~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~')
    check.add(curRoom)
...
while bool(curPath):
    curPathUpdate(player.currentRoom.id)
