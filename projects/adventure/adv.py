...
def bfs(move, room):
    # Create an empty Queue and enqueue A PATH FROM the starting vertex
    q = Queue()
    q.enqueue([move])
...
while len(graph) < len(roomGraph):
    while choose_dir(room, exits):
        direction = choose_dir(room, exits)
        player.travel(direction)
        traversalPath.append(direction)
        newroom = player.currentRoom.id
        graph[room][direction] = newroom
        if newroom not in visited:
            add_room(newroom)
            visited.add(newroom)
        graph[newroom][opp_dir(direction)] = room
        room = newroom
        exits = player.currentRoom.getExits()
    move = random_dir(room, exits)
    newdirs = bfs(move, room)
    traversalPath.extend(newdirs)
    for move in newdirs:
        player.travel(move)
    room = player.currentRoom.id
    exits = player.currentRoom.getExits()
...
# print('traversalPath', traversalPath)
    # print(graph, room)
# {0: {'n': 1, 's': '?', 'w': 7, 'e': '?'}, 1: {'n': 2, 's': 0}, 2: {'s': 1}, 5: {'n': 0, 's': 6}, 6: {'n': 5, 'w': 11}, 11: {'w': 10, 'e': 6}, 10: {'n': 9, 'e': 11}, 9: {'n': 8, 's': 10}, 8: {'s': 9, 'e': 7}, 7: {'w': 8, 'e': 0}} 0
