...
player = Player("Eddy", world.startingRoom)
...
stack = {}
stack[player.currentRoom.id] = [player.currentRoom, None]
...
while len(stack):
  # Get the last room in the stack (e.g. room id, room object, and room exit entry)

  rooms = list(stack.keys())
  current_room_id = rooms[-1]
  current_room = stack[current_room_id][0]
  current_room_exit = stack[current_room_id][1]

  print(f'stack {stack}')
  print(f'room id {current_room_id}')
  print(f'room exit {current_room_exit}')


  # If room was not previously visited or is it not the starting room?
  # Then, Add room exit entry to the traversal path.

  if current_room_exit != None and current_room_id not in visited:
    traversalPath.append(current_room_exit)
  
  print(f'path {traversalPath}')

  # Add rooom id to the visited set.
  ...
  # Should this room be popped off from the stack or should we continue traveling further?
  # If popremains true, then that means the current room has unvisited exits and rooms.
  ...
  for exit in exits:
    ...
    # If the next room was not visited previously, then add it to the stack?
    # Also, if this condition results to true, then the current room should not be removed from the stack.
  
    if room.id not in visited and room.id not in stack:
      pop = False
      stack[room.id] = [room, exit]
  
  # If pop is true, then remove the current room from the stack.

  if pop:
    rooms = list(stack.keys())
    next_room = stack[rooms[-1]]
    del stack[rooms[-1]]

    print(f'next room id {rooms[-1]}')

    # This will add the exit that was used to enter this room to the traversal path.
    
    if next_room[1] != None:
      exit = next_room[1]
      traversalPath.append(opposite_exit[exit])
    
    print(f'updated path {traversalPath}\n')
