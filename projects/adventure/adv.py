...
def customRecursion(traversalPath, roomNeighbors, newTraverse):
    correctPath = traversalPath.pop(0)
    nextCorrectPath = traversalPath.pop(0)
    print("Current path: ", correctPath)
    print("Target for next path: ", nextCorrectPath)
    while len(traversalPath) > 0:
        for i in roomNeighbors.items():
            #print("i is ", i)
            for j in i[1]:
                #Run through all rooms, looking for a match
                currentRoom = i[0]
                #print("current room being looked at: ", currentRoom)
                #print("correctPath", correctPath)
                #print("next path is, ", nextCorrectPath)
                neighborDirection = j
                #print("neighborDirection", neighborDirection)
                neighborNumber = i[1][j]
                #print("neighborNumnber: ", neighborNumber)
                #numberOfExits = len(i[1])
                if currentRoom == correctPath and neighborNumber is nextCorrectPath:
                    print("correct path found")
                    if len(traversalPath) == 0:
                        newTraverse.append(neighborDirection)
                        return newTraverse
                    #print("Match found at: ", currentRoom)
                    #print("correct path is, ", correctPath)
                    #print("next path is, ", nextCorrectPath)
                    print("length is: ", len(traversalPath))
                    newTraverse.append(neighborDirection)
                    # print("Current traversal array: ", newTraverse)
                    correctPath = nextCorrectPath
                    # print("Updated correct path: ", nextCorrectPath)
                    nextCorrectPath = traversalPath.pop(0)
                    print("length is: ", len(traversalPath))
                    # print("Next correct path should be: ", nextCorrectPath)
    return newTraverse
...
#traversalPath = newPathingRoutine
print(traversalPath)
# print("length", len(traversalPath))
...
# print("length visited", len(visited_rooms))

for move in traversalPath:
    #print("Current room: ", player.currentRoom)
    player.travel(move)
    visited_rooms.add(player.currentRoom)
    # print("length visited2", len(visited_rooms))
#print("visited rooms", visited_rooms)
if len(visited_rooms) == len(roomGraph):
    print(f"TESTS PASSED: {len(traversalPath)} moves, {len(visited_rooms)} rooms visited")
else:
    print("TESTS FAILED: INCOMPLETE TRAVERSAL")
    # print("roomgraph", len(roomGraph))
    # print("visited rooms", len(visited_rooms))
