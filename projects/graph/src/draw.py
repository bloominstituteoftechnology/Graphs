...
class BokehGraph:
    ...
    def show(self, graphstyle='random', graphcolor="connectedOnly", arrows=False, colorGroups=[]):
        ...
        if graphcolor == "viridis":
            graph.node_renderer.data_source.add(
                viridis(len(node_indices)), 'color')
            ...
            graph.node_renderer.data_source.add(
                [graphcolor] * len(node_indices), 'color')

        graph.node_renderer.glyph = Circle(radius=.05, fill_color='color')
        graph.edge_renderer.glyph = MultiLine(
            line_color="black", line_alpha=0.8, line_width=5)
