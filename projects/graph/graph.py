...
class Graph:
    ...
    def bft(self, starting_vertex):
        ...
        visted_ver = set()
        # while to_visit is not empty dequeue the first vertex on the queue if it has not been visited print
        while to_visit.size() > 0:   
            current_vertex = to_visit.dequeue()
            if current_vertex not in visted_ver:
                print(current_vertex)
                
                # mark as visited, by adding to visited_ver set add all unvisted neighbours to queue
