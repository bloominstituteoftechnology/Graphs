...
1. To create 100 users with an average of 10 friends each, how many times would you need to call `add_friendship()`? Why?
   ...
   500 (100 * 10 / 2). Each friendship created means that two users have an additional 1 friendship. For each user, we need to add avg_friends/2 friends to get the correct average.
